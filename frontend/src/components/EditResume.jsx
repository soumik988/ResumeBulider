...
const EditResume = () => {
    const { id: resumeId } = useParams(); // ✅ Use correct param name
    ...
    const [progress, setProgress] = useState(0); // Add this at the top with other useState
    ...
    function dataURLtoFile(dataurl, filename) {
        if (!dataurl || !dataurl.startsWith("data:image")) {
            throw new Error("Invalid data URL for thumbnail");
        }
    ...
    const calculateCompletion = () => {
        ...
        const { profileInfo, contactInfo, workExperience, education, skills, projects, certifications, languages, interests } = resumeData;

        // Profile Info
        totalFields += 3;
        if (profileInfo.fullName) completedFields++;
        if (profileInfo.designation) completedFields++;
        if (profileInfo.summary) completedFields++;

        // Contact Info
        totalFields += 2;
        if (contactInfo.email) completedFields++;
        if (contactInfo.phone) completedFields++;
        ...
        completedFields += interests.filter(i => i.trim() !== "").length;
    ...
    //take back to previous from or back to first page
    ...
    //it will be update resume image download resume
    ...
    return (
        <DashboardLayout>
                ...
                <div className={containerStyles.grid}>
                    <div className={containerStyles.formContainer}>
                        ...
                        <div className='p-4 sm:p-6'>
                            ...
                            <div className='flex lex-wrap items-center justify-end gap-3'>
                                ...
                                <button className={buttonStyles.next} onClick={validateAndNext} disabled={isLoading}>
                                    ...
                                    {currentPage === "additionalInfo" && <ArrowLeft size={16} className='rotate-100' />}
            ...
            <Modal
                isOpen={openPreviewModal}          // ✅ pass the boolean state, not setter
                ...
                actionBtnText={
                    isDownloading
                        ? "Generating..."
                        : downloadSuccess // ✅ correct camelCase
                            ? "Download!"
                            : "Download PDF"
            ...
            {/*NOW THUMBNAIL*/}
            <div style={{ position: "absolute", top: "-9999px" }} ref={thumbnailRef}>
